...
:: Boord Notes - double-click this file to update to the newest signed release
:: from GitHub, install any new dependencies, bring the database up to date,
:: and restart the service so the update takes effect.
::
:: This deliberately does NOT "git pull" a branch, which is what it used to
:: do. A branch pull trusts whoever can push to the repo, and the service this
:: restarts runs as SYSTEM - so a stolen GitHub token would mean code
:: execution on the farm server. Instead it checks out a tag carrying a GPG
:: signature from the release key, and refuses to update at all if that
:: signature is missing, broken, or made by any other key. Pushing code is
:: then not enough to ship it; you also have to hold the signing key.
::
:: This is the same scheme, and the same key, as Boord's update_server.bat and
:: Boord Owner's update_owner_server.bat - the three apps are one publisher,
:: and a farm that has already trusted that key does not have to decide again.
...
if "!CURTAG!"=="!NEWTAG!" (
    ...
    set "VERIFY_OUT=%TEMP%\boord_notes_verify.txt"
...
:: A cheap pre-change snapshot. Kept per-run rather than overwritten, because
:: "the copy from before the update that broke it" is worthless if the next
:: update overwrites it before anyone notices. This is only the database - the
:: nightly job in backend\backup.py is what captures the photos too.
...
if exist "%~dp0data\notebook.db" copy /y "%~dp0data\notebook.db" "%~dp0data\backups\notebook-before-!STAMP!.db" >nul
cd /d "%~dp0backend"
"%VENV_PY%" -c "from db import create_db_and_tables; create_db_and_tables()"
if errorlevel 1 (
    echo.
    echo *** THE DATABASE COULD NOT BE BROUGHT UP TO DATE ***
    echo.
    echo The server has not been restarted. Read the error above. A copy of
    echo the database was taken first - look for the newest notebook-before-*.db
...
if errorlevel 1 (
    echo     Could not find the "%TASK%" scheduled task. If you're running the
    echo     server manually in its own window instead, close and reopen that
    echo     window yourself to pick up the update. If this server was set up
    echo     before the rename, run install.bat once - it replaces the old
    echo     "Bekfontein Farm Notebook Server" task with this one.
    echo.
    pause
    exit /b 1
)
...
for /l %%i in (1,1,20) do (
    if not defined OK (
        ...
        powershell -NoProfile -Command "try { if ((Invoke-WebRequest -Uri 'http://localhost:%PORT%/app/' -UseBasicParsing -TimeoutSec 3).StatusCode -eq 200) { exit 0 } } catch {}; exit 1" && set OK=1
    )
)
if defined OK (
    echo     Server is back up on port %PORT%, running !NEWTAG!.
) else (
    echo     Server did not answer in 20s - run start_notes_server.bat in a
    echo     window to see why. Errors print there rather than being swallowed
    echo     by the Scheduled Task.
)

echo.
echo ==^> Done. Remember: the installed app on Andre's and Devin's phones
echo     still needs to be fully closed and reopened ^(not just backgrounded^)
echo     to pick up the new version. The version in the app's header is how
echo     you tell whether a phone actually has it.
echo.
pause
